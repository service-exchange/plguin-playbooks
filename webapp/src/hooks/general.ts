...
import {getMyTeams, getCurrentTeamId} from 'service-exchange-redux/selectors/entities/teams';
...
} from 'service-exchange-redux/selectors/entities/users';
import {
    getCurrentChannelId,
    getChannel as getChannelFromState,
} from 'service-exchange-redux/selectors/entities/channels';
import {DispatchFunc} from 'service-exchange-redux/types/actions';
import {getProfilesByIds, getProfilesInChannel, getProfilesInTeam} from 'service-exchange-redux/actions/users';
import {Client4} from 'service-exchange-redux/client';
import {getPost as getPostFromState} from 'service-exchange-redux/selectors/entities/posts';
import {UserProfile} from '@service-exchange/types/users';
import {getTeammateNameDisplaySetting} from 'service-exchange-redux/selectors/entities/preferences';
import {displayUsername} from 'service-exchange-redux/utils/user_utils';
import {ClientError} from '@service-exchange/client';

import {useHistory, useLocation} from 'react-router-dom';
import qs from 'qs';
import {haveITeamPermission} from 'service-exchange-webapp/packages/service-exchange-redux/src/selectors/entities/roles';
...
export const useExportLogAvailable = () => {
    ...
    return useSelector<GlobalState, boolean>((state) => Boolean(state.plugins?.plugins?.['com.service-exchange.plugin-channel-export']));
};
