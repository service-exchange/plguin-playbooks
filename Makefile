...
GO_TEST_FLAGS ?= -race
GO_BUILD_FLAGS ?=
MM_UTILITIES_DIR ?= ../service-exchange-utilities
DLV_DEBUG_PORT := 2346
...
i18n-extract-server:
ifneq ($(HAS_SERVER),)
	$(GO) install -modfile=go.tools.mod github.com/service-exchange/service-exchange-utilities/mmgotool
	mkdir -p server/i18n
...
ifndef STARTERTEMPLATE_PATH
	...
	@echo Set STARTERTEMPLATE_PATH to a local clone of https://github.com/service-exchange/service-exchange-plugin-starter-template and retry.
	@exit 1
endif
	cd ${STARTERTEMPLATE_PATH} && go run ./build/sync/main.go ./build/sync/plan.yml $(PWD)

# Help documentation ?? la https://marmelab.com/blog/2016/02/29/auto-documented-makefile.html
help:
