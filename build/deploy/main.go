...
// or by copying the plugin bundle into a sibling service-exchange-server/plugin directory.
package main
...
func deploy() error {
	...
	copyTargetDirectory, _ := filepath.Abs("../service-exchange-server")
	...
	log.Printf("Installing plugin to service-exchange-server found in %s.", copyTargetDirectory)
...
// copyPlugin attempts to install a plugin by copying it to a sibling ../service-exchange-server/plugin
