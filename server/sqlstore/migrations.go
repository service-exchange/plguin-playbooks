...
var migrations = []Migration{
	...
	},
	{
		...
		migrationFunc: func(e sqlx.Ext, sqlStore *SQLStore) error {
			...
			if e.DriverName() == model.DatabaseDriverMysql {
				if _, err := e.Exec("UPDATE IGNORE PluginKeyValueStore SET PluginId='playbooks' WHERE PluginId='com.service-exchange.plugin-incident-management'"); err != nil {
					logrus.WithError(err).Error("failed to migrate KV store plugin id")
				}
			} else {

				if _, err := e.Exec("UPDATE PluginKeyValueStore k SET PluginId='playbooks' WHERE PluginId='com.service-exchange.plugin-incident-management' AND NOT EXISTS ( SELECT 1 FROM PluginKeyValueStore WHERE PluginId='playbooks' AND PKey = k.PKey )"); err != nil {
					logrus.WithError(err).Error("failed to migrate KV store plugin id")
				}
			}
